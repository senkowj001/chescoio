...
- **`brands/0004_contact_email_routing.py` — new migration (contact email routing).** John asked that contact submissions send FROM noreply@chesco.io TO admin@chesco.io (Hostinger misbehaves when From==To). The contact view and the order emails send `from_email=brand.from_email` / `recipient_list=[brand.support_email]`, so this is a two-field Brand change, not a settings change: `from_email → noreply@chesco.io`, `support_email → admin@chesco.io`. Data migration mirroring `0003_recolor_chesco` (reversible to hello@/hello@). Blast radius: `from_email` is the FROM for ALL transactional mail (contact + order confirmation + shipped + admin failure alert), now all noreply@; `support_email` is the TO for the contact + order-failure alerts (internal — admin@ is correct) AND the customer-facing support address in the footer/order emails, so admin@ is now publicly visible. Flagged to John: if he wants a friendlier public address (hello@/support@) while keeping internal notifications on admin@, that's a separate small change (dedicated notification recipient) — not done unless requested. Reply-To is not set on the contact notification (a reply hits noreply@) — offered as a follow-up. **Operational:** the noreply@ and admin@ mailboxes must exist at Hostinger, with `EMAIL_HOST_USER`/`EMAIL_HOST_PASSWORD` set to the noreply@ mailbox (the From should match the authenticated sender).
- **Required “no return policy” acknowledgment at checkout (3 files).** Per John: a required checkbox the customer must tick to purchase, with a hyperlink reading exactly “no return policy” linking to the returns page. Stripe's hosted checkout can't render a custom required checkbox with arbitrary link text (its only native consent checkbox is a generic “Terms of Service” link pointing at a single dashboard URL), so the checkbox lives on our cart page instead. (a) `templates/orders/_cart_contents.html`: a `required` checkbox in the checkout form — label “I understand all sales are final and have read the [no return policy]” linking to `core:returns` (opens in a new tab). (b) `orders/views.py::checkout_start`: server-side backstop — rejects the POST (redirect + message) if `agree_returns` is absent, so the `required` attribute can't be skipped by a crafted request. (c) `orders/checkout_services.py`: stamps `returns_policy_accepted: 'true'` onto the Stripe session metadata so the acknowledgment shows on the payment in the Stripe dashboard (chargeback evidence). **HARD DEPENDENCY: the returns StaticPage must be published (is_published=True) or the “no return policy” link 404s for the public.** Not auto-published — the sprint requires John's explicit sign-off on returns copy; John reviews + publishes it. Optional future step: persist the acceptance on the Order (needs a field + migration); the Stripe metadata is the evidence trail for v1.
- **`core/0003_contact_email_to_admin.py` — new migration (contact email fix).** hello@chesco.io was never a real mailbox; admin@chesco.io is the brand's actual address. The Sprint 5 seed hard-coded hello@ throughout the page copy, including the already-published about + size-guide pages. Data migration replaces hello@chesco.io → admin@chesco.io in every StaticPage `content` / `meta_description` for the brand. Layered on top of the seed (0002) rather than editing it, so it fixes already-migrated DBs on `migrate`; plain string replace, so admin-edited pages are left untouched.
- **Admin moved from `/admin/` to `/ct-ops/` (2 files).** John's request, to cut automated bot probing of the default admin path (bots hitting `/admin/` were generating noise; note the accompanying 500s are the separate DB-connect-timeout issue, not caused by the probes). `chescoio/urls.py`: `path('admin/', …)` → `path('ct-ops/', …)`. Admin's internal URLs reverse via the `admin:` namespace relative to the mount point, so nothing inside admin changes. `core/views.py::robots_txt`: dropped the now-stale `Disallow: /admin/` line and deliberately did NOT add `/ct-ops/` (robots.txt is public; a Disallow line would advertise the admin location). This is obscurity only — not a substitute for a strong admin password / 2FA. John must bookmark `/ct-ops/` and update any saved `/admin/` link.
- **Fixed add-to-cart 500 for multiple products on one blueprint (2 files).** Adding a product to the cart 500'd with `Variant.MultipleObjectsReturned: … returned 3`. Not corrupt data: `printify_variant_id` is a Printify *blueprint*-level ID (e.g. “M / Black” on the Unisex Heavy Cotton Tee) shared by every product built on that blueprint, so once the brand had 3 tees on one blueprint, `cart_add`'s `Variant.objects.get(printify_variant_id=…, product__brand=brand)` matched 3 rows. `(product, printify_variant_id)` is unique_together, so the fix scopes the lookup to the product. `templates/catalog/product_detail.html`: the add-to-cart button already carried `data-product-id`; added `product_id` to its `hx-vals`. `orders/views.py::cart_add`: parse `product_id` (400 if missing) and add `product_id=` to the `.get()`, making the match exact. The variant picker + image-swap logic (both keyed on printify_variant_id) are untouched. No data cleanup — the 3 rows are legitimately different products.
